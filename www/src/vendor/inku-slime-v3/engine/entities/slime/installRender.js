...
import { updateAndDrawPhysicsAccessory } from './installPhysicsAccessories.js';

const PHYSICS_ACCESSORIES = new Set(['silk_ribbon', 'spectral_tail', 'spring_antenna']);
...
export function installRender(Slime) {
  ...
  Slime.prototype.draw = function() {
    ...
    if (PHYSICS_ACCESSORIES.has(this.accessory)) {
      ctx.restore();
      updateAndDrawPhysicsAccessory(this, visualX, visualY);
      return;
    }
  ...
  Slime.prototype._drawAccessoryShape = function(acc) {
    switch (acc) {
      ...
      case 'leaf': {
        // Gradient-filled leaf with central vein and secondary veins
        const lg = ctx.createLinearGradient(-2,10,18,-20);
        lg.addColorStop(0,'#1a7f3c'); lg.addColorStop(0.4,'#2dc653'); lg.addColorStop(1,'#52e87a');
        ctx.fillStyle = lg;
        ctx.beginPath();
        ctx.moveTo(-2,10);
        ctx.bezierCurveTo(8,4, 22,-6, 18,-22);
        ctx.bezierCurveTo(14,-30, 4,-22, -2,10);
        ctx.closePath(); ctx.fill();
        // Rim
        ctx.strokeStyle='rgba(10,80,30,0.35)'; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(-2,10); ctx.bezierCurveTo(8,4,22,-6,18,-22); ctx.bezierCurveTo(14,-30,4,-22,-2,10); ctx.closePath(); ctx.stroke();
        // Central vein
        ctx.strokeStyle='rgba(10,80,30,0.55)'; ctx.lineWidth=1.6; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(-2,9); ctx.bezierCurveTo(6,0,14,-10,17,-20); ctx.stroke();
        // Secondary veins
        ctx.strokeStyle='rgba(10,80,30,0.28)'; ctx.lineWidth=0.9;
        [[4,2,10,-4],[8,-5,14,-12],[11,-11,17,-18]].forEach(([x1,y1,x2,y2]) => {
          ctx.beginPath(); ctx.moveTo(x1,y1); ctx.lineTo(x2,y2); ctx.stroke();
        });
        // Light sheen
        ctx.fillStyle='rgba(255,255,255,0.12)';
        ctx.beginPath(); ctx.moveTo(2,4); ctx.bezierCurveTo(10,-2,18,-12,16,-20); ctx.bezierCurveTo(12,-14,6,-4,2,4); ctx.closePath(); ctx.fill();
        ctx.lineCap='butt';
        break;
      }
      case 'sprout':
      ...
      case 'clover': {
        const cg = ctx.createRadialGradient(0,-4,0,0,-4,14);
        cg.addColorStop(0,'#6ed98a'); cg.addColorStop(0.5,'#52b788'); cg.addColorStop(1,'#2d8a5e');
        ctx.fillStyle = cg;
        [[-5,-9],[5,-9],[-5,-1],[5,-1]].forEach(([x,y]) => {
          ctx.beginPath(); ctx.arc(x,y,6,0,Math.PI*2); ctx.fill();
        });
        ctx.strokeStyle='rgba(255,255,255,0.22)'; ctx.lineWidth=1.2; ctx.lineCap='round';
        [[-5,-9],[5,-9],[-5,-1],[5,-1]].forEach(([x,y]) => {
          ctx.beginPath(); ctx.moveTo(x,y+2); ctx.lineTo(x-1.5,y-3); ctx.stroke();
        });
        ctx.strokeStyle='#2d6a4f'; ctx.lineWidth=2.2; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(0,2); ctx.quadraticCurveTo(3,9,8,14); ctx.stroke();
        ctx.lineCap='butt';
        break;
      }
      case 'feather': {
        // Full layered quill feather with barbs
        const fg = ctx.createLinearGradient(-2,12,8,-22);
        fg.addColorStop(0,'#d4c5a0'); fg.addColorStop(0.5,'#ece8d8'); fg.addColorStop(1,'#f5f2e8');
        ctx.fillStyle = fg;
        ctx.beginPath();
        ctx.moveTo(-2,12);
        ctx.bezierCurveTo(-8,4, -10,-10, 4,-22);
        ctx.bezierCurveTo(14,-26, 16,-14, 8,2);
        ctx.bezierCurveTo(5,8, 1,10, -2,12);
        ctx.closePath(); ctx.fill();
        // Rim shadow
        ctx.strokeStyle='rgba(140,120,80,0.35)'; ctx.lineWidth=1;
        ctx.beginPath();
        ctx.moveTo(-2,12); ctx.bezierCurveTo(-8,4,-10,-10,4,-22); ctx.bezierCurveTo(14,-26,16,-14,8,2); ctx.bezierCurveTo(5,8,1,10,-2,12); ctx.closePath(); ctx.stroke();
        // Central quill
        ctx.strokeStyle='rgba(120,100,60,0.65)'; ctx.lineWidth=1.8; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(-1,10); ctx.bezierCurveTo(2,2, 6,-10, 6,-20); ctx.stroke();
        // Barb lines (left side)
        ctx.strokeStyle='rgba(160,140,100,0.3)'; ctx.lineWidth=0.9;
        [[-4,6],[-5,0],[-4,-6],[-2,-12],[1,-17]].forEach(([x,y],i) => {
          ctx.beginPath(); ctx.moveTo(6-i*0.4,-20+i*6); ctx.lineTo(x,y); ctx.stroke();
        });
        // Barb lines (right side)
        [[12,-2],[14,-8],[12,-14],[8,-19]].forEach(([x,y],i) => {
          ctx.beginPath(); ctx.moveTo(6,-20+i*5+4); ctx.lineTo(x,y); ctx.stroke();
        });
        // Tip sheen
        ctx.fillStyle='rgba(255,255,255,0.22)';
        ctx.beginPath(); ctx.moveTo(4,-18); ctx.bezierCurveTo(12,-22,14,-14,10,-4); ctx.bezierCurveTo(8,-10,6,-16,4,-18); ctx.closePath(); ctx.fill();
        ctx.lineCap='butt';
        break;
      }
      case 'shell_pin':
        ...
        break;
      case 'bone_pin': {
        const bg = ctx.createLinearGradient(-12,12,12,-12);
        bg.addColorStop(0,'#e8eaed'); bg.addColorStop(0.5,'#ffffff'); bg.addColorStop(1,'#cdd0d6');
        ctx.strokeStyle = bg; ctx.lineWidth=4; ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(-9,9); ctx.lineTo(9,-9); ctx.stroke();
        [[-10,10],[-6,6],[10,-10],[6,-6]].forEach(([x,y],i) => {
          const r = i < 2 ? 4.5 : 3.5;
          const beg = ctx.createRadialGradient(x-1,y-1,0,x,y,r);
          beg.addColorStop(0,'#ffffff'); beg.addColorStop(1,'#c8cdd4');
          ctx.fillStyle = beg;
      ...
      case 'cat_ears': {
        // Outer ear — soft rounded triangles, NOT sharp points
        const drawEar = (side) => {
          const sx = side;
          // Outer shape: rounder tip via bezier
          const earG = ctx.createLinearGradient(sx*18,-26,sx*6,-2);
          earG.addColorStop(0,'#f7d6e0');
          earG.addColorStop(0.5,'#f0bcd1');
          earG.addColorStop(1,'#e8a0bc');
          ctx.fillStyle = earG;
          ctx.beginPath();
          ctx.moveTo(sx*6,-2);
          ctx.bezierCurveTo(sx*16,-4, sx*24,-12, sx*20,-26);   // outer slope
          ctx.bezierCurveTo(sx*18,-30, sx*14,-28, sx*12,-24);  // rounded tip
          ctx.bezierCurveTo(sx*8,-18, sx*4,-10, sx*6,-2);      // inner slope
          ctx.closePath();
          ctx.fill();
          // Soft drop shadow
          ctx.shadowColor='rgba(180,100,140,0.18)'; ctx.shadowBlur=4;
          ctx.strokeStyle='rgba(210,140,170,0.4)'; ctx.lineWidth=1;
          ctx.beginPath();
          ctx.moveTo(sx*6,-2);
          ctx.bezierCurveTo(sx*16,-4, sx*24,-12, sx*20,-26);
          ctx.bezierCurveTo(sx*18,-30, sx*14,-28, sx*12,-24);
          ctx.bezierCurveTo(sx*8,-18, sx*4,-10, sx*6,-2);
          ctx.closePath();
          ctx.stroke();
          ctx.shadowBlur = 0;
          // Inner ear — warm pink, inset
          const innerG = ctx.createLinearGradient(sx*14,-22,sx*8,-6);
          innerG.addColorStop(0,'#ff9ec4'); innerG.addColorStop(1,'#ffcfe0');
          ctx.fillStyle = innerG;
          ctx.beginPath();
          ctx.moveTo(sx*8,-5);
          ctx.bezierCurveTo(sx*14,-8, sx*20,-16, sx*17,-23);
          ctx.bezierCurveTo(sx*15,-26, sx*13,-24, sx*11,-20);
          ctx.bezierCurveTo(sx*8,-14, sx*6,-8, sx*8,-5);
          ctx.closePath();
          ctx.fill();
          // Specular on tip
          ctx.fillStyle='rgba(255,255,255,0.45)';
          ctx.beginPath(); ctx.ellipse(sx*17,-22,2,3.5,-0.4*sx,0,Math.PI*2); ctx.fill();
        };
        drawEar(-1); drawEar(1);
        break;
      }
      ...
      case 'fox_ears': {
        const drawFoxEar = (side) => {
          const sx = side;
          // Outer ear — orange, tall, rounded tip
          const og = ctx.createLinearGradient(sx*18,-30,sx*6,-2);
          og.addColorStop(0,'#e8722a'); og.addColorStop(0.5,'#d4622a'); og.addColorStop(1,'#b84a18');
          ctx.fillStyle = og;
          ctx.beginPath();
          ctx.moveTo(sx*5,-2);
          ctx.bezierCurveTo(sx*14,-4, sx*22,-10, sx*20,-28);
          ctx.bezierCurveTo(sx*18,-32, sx*14,-30, sx*12,-26);
          ctx.bezierCurveTo(sx*8,-18, sx*3,-10, sx*5,-2);
          ctx.closePath(); ctx.fill();
          // White inner ear
          ctx.fillStyle='rgba(255,255,255,0.92)';
          ctx.beginPath();
          ctx.moveTo(sx*7,-4);
          ctx.bezierCurveTo(sx*13,-7, sx*19,-14, sx*17,-26);
          ctx.bezierCurveTo(sx*15,-28, sx*13,-27, sx*12,-24);
          ctx.bezierCurveTo(sx*9,-16, sx*6,-9, sx*7,-4);
          ctx.closePath(); ctx.fill();
          // Inner pink
          const ig = ctx.createLinearGradient(sx*14,-23,sx*9,-6);
          ig.addColorStop(0,'#ffb09a'); ig.addColorStop(1,'#ffd5c8');
          ctx.fillStyle = ig;
          ctx.beginPath();
          ctx.moveTo(sx*8,-5);
          ctx.bezierCurveTo(sx*13,-8, sx*17,-15, sx*15,-24);
          ctx.bezierCurveTo(sx*14,-26, sx*12,-25, sx*11,-22);
          ctx.bezierCurveTo(sx*9,-15, sx*7,-9, sx*8,-5);
          ctx.closePath(); ctx.fill();
          // Tip specular
          ctx.fillStyle='rgba(255,200,150,0.5)';
          ctx.beginPath(); ctx.ellipse(sx*17,-22,2,3.5,-0.3*sx,0,Math.PI*2); ctx.fill();
        };
        drawFoxEar(-1); drawFoxEar(1);
        break;
      }
      ...
      case 'bunny_ears': {
        const drawBunnyEar = (side) => {
          const sx = side;
          // Outer ear — tall oval with slight lean
          const tilt = sx * 0.15;
          const eg = ctx.createLinearGradient(sx*8,-38, sx*12,-2);
          eg.addColorStop(0,'#f9d0df'); eg.addColorStop(0.5,'#f2b8cc'); eg.addColorStop(1,'#e8a0bc');
          ctx.fillStyle = eg;
          ctx.beginPath();
          ctx.moveTo(sx*6,-2);
          ctx.bezierCurveTo(sx*2,-8,  sx*6-tilt*30,-30,  sx*10,-38);  // left slope
          ctx.bezierCurveTo(sx*14,-38, sx*18-tilt*10,-28, sx*14,-2);   // right slope
          ctx.closePath(); ctx.fill();
          // Border shadow
          ctx.strokeStyle='rgba(200,130,165,0.3)'; ctx.lineWidth=1.2;
          ctx.beginPath();
          ctx.moveTo(sx*6,-2);
          ctx.bezierCurveTo(sx*2,-8, sx*6-tilt*30,-30, sx*10,-38);
          ctx.bezierCurveTo(sx*14,-38, sx*18-tilt*10,-28, sx*14,-2);
          ctx.closePath(); ctx.stroke();
          // Inner ear — warm blush
          const ig = ctx.createLinearGradient(sx*8,-35, sx*12,-6);
          ig.addColorStop(0,'#ffb3c8'); ig.addColorStop(0.6,'#ffc8d8'); ig.addColorStop(1,'#ffe0ea');
          ctx.fillStyle = ig;
          ctx.beginPath();
          ctx.moveTo(sx*8,-6);
          ctx.bezierCurveTo(sx*6,-12, sx*8-tilt*20,-30, sx*10,-34);
          ctx.bezierCurveTo(sx*12,-34, sx*14-tilt*8,-26, sx*12,-6);
          ctx.closePath(); ctx.fill();
          // Specular
          ctx.fillStyle='rgba(255,255,255,0.4)';
          ctx.beginPath(); ctx.ellipse(sx*9,-28,1.5,4.5,-tilt*0.3,0,Math.PI*2); ctx.fill();
        };
        drawBunnyEar(-1); drawBunnyEar(1);
        break;
      }
