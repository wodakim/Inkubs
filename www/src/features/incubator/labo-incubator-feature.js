...
export function createLaboIncubatorFeature({ store } = {}) {
    ...
    function handleVisibilityChange() {
        if (document.hidden) {
            ...
            // Ne reprendre l'orchestrateur que si le labo est actif (pas suspendu).
            // La condition précédente `(!isSuspended || orchestrator)` était toujours
            // vraie car orchestrator est un objet — elle causait un resume même quand
            // on revenait sur la prairie, provoquant le bug du "slime fantôme".
            if (!isSuspended) {
                orchestrator?.resume?.();
            }
        }
