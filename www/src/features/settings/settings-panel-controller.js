...
import { SlimeSoundEngine } from '../prairie/slime-sound-engine.js';
...
export function createSettingsPanelController() {
    ...
    SLIDERS.forEach(([id, k]) => {
        ...
        el?.addEventListener('input', () => {
            ...
            if (k === 'master' || k === 'music') {
                SlimeSoundEngine.setMusicVolume(masterVol * (settings.music / 100));
            }
            if (k === 'master' || k === 'sfx') {
                SlimeSoundEngine.setSfxVolume(masterVol * (settings.sfx / 100));
            }
        });
    });
