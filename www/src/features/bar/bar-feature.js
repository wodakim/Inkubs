...
export function createBarFeature({ store }) {
    ...
    function hideFeature() {
        if (!root) return;
        root.hidden = false; // garder dans le DOM
    ...
    function buildScene(mount) {
        // Ne construire qu'une seule fois
        if (root) return;
        ...
        // Attacher une seule fois dans le DOM
    ...
    return {
        ...
        resume({ mount }) {
            host = mount;
            // Réattacher si le DOM a été nettoyé (ne devrait pas arriver, mais sécurité)
        ...
        suspend() {
            // Cacher visuellement sans détruire — le jeu continue en arrière-plan
            dialogue?.reset();
            canTalk = true;
            questPanel?.close?.();
            hideFeature();
